      ...
      try {
        ...
        if (result.success) {
          ...
          const postsArray = postsData ? 
            (Array.isArray(postsData) ? postsData : [postsData]) : 
            [];
          ...
          {posts.length > 0 ? (
            posts.map((post, index) => (
              <div key={index} className="post-card">
                <img 
                  src={post.file || bannerImage} 
                  alt={post.fileName || 'Post image'} 
                  className="post-image" 
                />
                <h2 className="post-title">{post.fileName || 'Untitled Post'}</h2>
                <p className="post-price">{post.price || 'Price not available'}</p>
                <p className="post-location">Total Quantity: {post.quantity || 'N/A'}</p>
                <p className="post-category">{post.category || 'Uncategorized'}</p>
